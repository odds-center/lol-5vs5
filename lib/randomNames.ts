...
/**
 * 빈 닉네임 슬롯에 풀에서 중복 없이 랜덤 이름을 채움.
 * 이미 사용 중인 이름·챔피언은 사용하지 않음.
 */
export function fillEmptyNames(players: Player[]): Player[] {
  const used = new Set(players.map((p) => p.name.trim()).filter(Boolean));
  const available = shuffle(CHAMPION_NAMES.filter((n) => !used.has(n)));

  if (available.length < 10 - used.size) {
    // 풀 부족 시 나머지는 "참가자 N" 형태
    let fallback = 1;
    return players.map((p) => {
      if (p.name.trim() !== '') return p;
      const name = available.length > 0 ? available.shift()! : `참가자 ${fallback++}`;
      used.add(name);
      return { ...p, name };
    });
  }
