# Discord 봇 연동 계획 — 음성 채널 참가자 자동 불러오기

> 상태: **1단계 구현·배포 완료** (실제 내전 서버에서 `/내전` 테스트만 남음)
> 목표: 디스코드 음성 채널에 모인 사람들의 이름을 사이트 참가자 명단에 자동으로 채워 넣는다.
>
> 봇은 이 레포가 아니라 **별도 레포 [`odds-center/Discord-Bot`](https://github.com/odds-center/Discord-Bot)의 `lol/` 폴더**에 있고 Railway에 배포한다.
> 사이트 쪽 변경(§3.4)은 이 레포에 있다.

---

## 1. 결론 요약

- **가능하다.** 다만 현재 사이트는 Vercel 정적 배포 + localStorage 전용이라, 봇과 사이트가 실시간으로 데이터를 주고받으려면 서버·DB가 새로 필요하다.
- 그래서 1단계는 **서버/DB 없이** 동작하는 방식으로 한다.
  1. 디스코드에서 `/내전` 슬래시 명령 실행
  2. 봇이 **명령을 실행한 사람이 들어가 있는 음성 채널**의 멤버 목록을 읽음
  3. 봇이 참가자 이름이 담긴 **사이트 링크**를 본인에게만 보이게(ephemeral) 답장
  4. 링크를 열면 사이트가 URL의 이름을 읽어 확인 모달을 띄우고, 10명을 맞춘 뒤 참가자 칸에 채움 (MMR은 이전에 입력한 값을 닉네임 기준으로 자동 복원)
- 봇은 음성 상태를 받기 위해 **Gateway(WebSocket)에 상시 접속**해야 하므로 Vercel 서버리스에서는 돌릴 수 없다. 봇만 별도로 상시 실행 환경에 배포한다.

---

## 2. 왜 이 구조인가

| 선택지 | 장점 | 단점 | 판단 |
|--------|------|------|------|
| **A. 봇이 링크 생성 → 사이트가 URL로 불러오기** | 사이트는 지금처럼 정적·localStorage 유지, DB 불필요, 개인정보 서버 저장 없음 | 링크를 한 번 눌러야 함 | **1단계 채택** |
| B. 사이트에 "디스코드에서 불러오기" 버튼 → API 호출 | 버튼 한 번으로 끝 | 디스코드 로그인(OAuth)·API 서버·봇과의 상태 공유 필요 | 2단계 후보 |
| C. 봇이 디스코드 안에서 팀까지 나눠서 임베드로 출력 | 사이트를 안 열어도 됨 | MMR 저장소(DB)를 봇 쪽에 따로 둬야 함 | 3단계 후보 |

HTTP Interactions(서버리스 엔드포인트)만으로는 "특정 음성 채널에 누가 있는지"를 목록으로 조회하는 REST API가 없어서, 음성 상태 캐시를 받는 **Gateway 봇**이 필요하다.

---

## 3. 1단계 상세 설계

### 3.1 사용자 흐름

1. 내전 인원이 음성 채널에 모인다.
2. 방장이 텍스트 채널에서 `/내전` 입력.
3. 봇 응답 (본인에게만 보임):
   - 음성 채널에 없으면: "먼저 음성 채널에 들어가 주세요."
   - 음성 채널 인원을 **추리지 않고 전부** 이름 목록 + **[사이트에서 열기]** 링크 버튼으로 보낸다 (최대 30명)
4. 링크를 열면 사이트가 확인 모달을 띄운다. **10명을 넘으면 모달에서 빼서 인원을 맞춘 뒤** 참가자 칸을 채운다.

> 디스코드 셀렉트 메뉴로 10명을 고르게 하는 방식도 검토했지만, 메뉴에서 한 명씩 클릭하는 것보다
> "일단 다 받고 사이트에서 빼는" 편이 빨라서 채택하지 않았다.

### 3.2 봇 (별도 레포 `odds-center/Discord-Bot`, `lol/` 폴더)

봇을 하위 디렉터리가 아니라 별도 레포로 둔 이유: Railway는 레포 루트를 그대로 빌드하므로 분리가 단순하고,
사이트 커밋마다 봇이 재배포되는 일도 없다.

- 런타임: Node.js 18+, TypeScript, `discord.js` v14 (설치·타입체크 검증: 14.27.0)
- Gateway Intents: `Guilds`, `GuildVoiceStates` (둘 다 **비특권** 인텐트 → 별도 승인 불필요)
- 권한(초대 링크): scope `bot`, `applications.commands` / 권한 `View Channels`만. 음성 채널 **접속(Connect)은 필요 없음** (상태만 읽음)
- 명령: `/내전` — 선택 옵션 `채널:`로 내가 있는 채널이 아닌 다른 음성 채널도 지정 가능
- 처리 로직:
  1. `interaction.guild.voiceStates.cache.get(interaction.user.id)?.channel` 로 호출자의 음성 채널 확인
  2. `channel.members` 에서 봇 계정 제외
  3. 표시 이름 = 서버 닉네임 → 전역 표시 이름 → 사용자명 순 (`member.displayName`)
  4. 캐시에 멤버 정보가 없으면 `guild.members.fetch({ user: ids })` 로 보충 (ID 지정 조회는 특권 인텐트 불필요)
  5. 이름 배열을 링크로 인코딩 후 `ephemeral` 응답
- 환경 변수: `DISCORD_TOKEN`, `DISCORD_APP_ID`, `SITE_URL`, `DEV_GUILD_ID`(선택)
- 명령 등록 스크립트: `scripts/register-commands.ts` — `DEV_GUILD_ID`가 있으면 길드 명령으로 즉시 반영,
  없으면 전역 등록(반영까지 최대 1시간)
- 제약: **링크 버튼의 URL은 512자**까지다. 인원이 많거나 닉네임이 길면 넘을 수 있어,
  한도를 넘으면 버튼 대신 메시지 본문에 링크를 넣는다 (본문은 2000자)
- 한 링크에 담는 인원 상한은 30명. 그보다 많으면 앞의 30명만 담고 메시지로 알린다

### 3.3 링크 형식

```
https://lol-5vs5.vercel.app/?import=<base64url(JSON)>
```

```json
{ "v": 1, "source": "discord", "names": ["닉네임1", "닉네임2", "..."] }
```

- 이름 10개 기준 URL은 수백 자 수준. 다만 **링크 버튼은 512자 제한**이라 인원·닉네임이 길면 본문 링크로 대체된다
- 버전 필드 `v`로 형식 변경에 대비
- 서버에 아무것도 저장하지 않으며, 링크는 요청한 본인에게만 보이는 메시지로 전달

### 3.4 사이트 변경 사항

| 파일 | 변경 |
|------|------|
| `lib/importLink.ts` (신규) | `?import=` 파싱·검증 (base64url 디코드, 배열 길이 ≤ 30, 이름 trim·32자 제한). `PARTICIPANT_SLOTS`(10)도 여기서 관리 |
| `lib/storage.ts` | `lol-5vs5-mmr-by-name` 키 추가: 닉네임 → 마지막 MMR 기록/조회 |
| `app/page.tsx` | 마운트 시 `import` 파라미터가 있으면 확인 모달 → 참가자 슬롯 채우기 → `history.replaceState`로 파라미터 제거 |
| `components/ImportConfirmModal.tsx` (신규) | 덮어쓰기 확인 모달. 이름별 빼기(✕), `N / 10` 카운터, 10명 초과면 불러오기 비활성화. ESC·배경 클릭으로 취소 |
| `lib/discord.ts` (신규) | 봇 초대 URL 생성. `NEXT_PUBLIC_DISCORD_APP_ID`가 없으면 `null` |
| `components/DiscordInviteButton.tsx` (신규) | 참가자 탭의 "디스코드에 봇 추가" 버튼. 앱 ID가 없으면 렌더링하지 않음 |
| `lib/i18n.ts` | 불러오기 확인 문구, 초대 버튼 문구 (ko/en) |

동작 규칙:
- 불러온 인원이 10명 미만이면 앞 칸부터 채우고 나머지는 비워 둔다 (기존처럼 빈 이름은 팀 나누기 때 챔피언 이름으로 자동 채움)
- **10명을 넘으면 모달에서 뺄 때까지 불러오기를 막는다.** 임의로 앞 10명만 자르지 않는다 —
  누가 빠질지는 사람이 정해야 한다
- 닉네임별 MMR 기록이 있으면 그 값을, 없으면 기본값 1000을 넣는다
- 기존 명단·같은 팀 지정·팀 결과는 확인 후에만 덮어쓴다 (같은 팀 지정은 슬롯 기준이라 초기화)
- **닉네임별 MMR은 입력할 때마다가 아니라 「팀 나누기」·「다시 나누기」 시점에 기록한다.**
  키 입력마다 저장하면 "김", "김철", "김철수" 같은 중간 입력이 전부 남는다
- **`lol-5vs5-mmr-by-name`은 「완전 초기화」로 지우지 않는다.** 지우면 MMR 복원 기능이 매번 리셋된다

### 3.5 배포

- 사이트: 기존 Vercel 그대로. 환경 변수 `NEXT_PUBLIC_DISCORD_APP_ID`를 넣으면 초대 버튼이 나타난다
- 봇: **Railway** (별도 레포 연결, **Root Directory = `lol`**, 실행 명령 `npm start`).
  봇이 늘어날 수 있어 레포를 프로젝트별 폴더로 나눠 뒀으므로, 서비스마다 Root Directory를 지정해야 한다.
  Watch Paths를 `lol/**`로 걸지 않으면 다른 폴더를 고쳐도 재배포된다.
  WebSocket 하나만 붙들고 대기하므로 메모리 100~200MB·CPU 거의 0 → Hobby 플랜 크레딧 안에서 끝난다.
  Fly.io, Render, 개인 서버/라즈베리파이 + `pm2`도 동일하게 가능
- 디스코드 개발자 포털: 애플리케이션 생성 → Bot 토큰 발급 → 서버 초대
  (초대 링크는 사이트의 "디스코드에 봇 추가" 버튼이 만들어 준다)

---

## 4. 작업 순서 (체크리스트)

- [x] 봇 레포 스캐폴딩 (`discord.js`, `tsx`, 환경 변수 로딩) — `odds-center/Discord-Bot`의 `lol/`
- [x] `/내전` 명령 등록 스크립트
- [x] 음성 채널 멤버 조회 (인원 제한 없이 전부 전달)
- [x] 링크 인코딩 및 ephemeral 응답
- [x] 사이트: `?import=` 파서 + 확인 모달 + 슬롯 채우기
- [x] 사이트: 닉네임별 MMR 기억·복원
- [x] 사이트: 봇 초대 버튼 (앱 ID 없으면 숨김)
- [x] 사이트: 10명 초과 시 모달에서 빼기
- [x] 디스코드 애플리케이션·봇 생성, 토큰 발급 (`Legends Civil War`)
- [x] 봇 레포를 GitHub에 올리고 Railway 연결 (프로젝트 `Discord-Bot` / 서비스 `lol`)
- [x] Vercel에 `NEXT_PUBLIC_DISCORD_APP_ID` 설정
- [ ] 봇을 내전 서버에 초대하고 실제로 `/내전` 테스트 — **사람이 해야 함**

---

## 5. 이후 단계 아이디어

- **2단계 (선택 B)**: 디스코드 OAuth 로그인 + 작은 API(예: Vercel Functions + KV)로 "불러오기" 버튼 한 번에 처리
- **3단계 (선택 C)**: `lib/teamAlgorithm.ts`, `lib/roleAssignment.ts`를 봇과 공유해 `/팀나누기` 결과를 디스코드 임베드로 바로 출력, 팀별 음성 채널로 자동 이동(`Move Members` 권한 필요)
- MMR을 봇 쪽 DB에 저장하면 사이트 없이도 밸런스 팀 구성 가능

---

## 6. 확인이 필요한 점

- ~~봇을 상시 실행할 호스팅 환경~~ → **Railway로 결정**
- 디스코드에 보이는 이름을 그대로 쓸지, 롤 닉네임(라이엇 ID)과 매핑할지 — 매핑하려면 `/닉네임등록` 같은 명령과 저장소가 추가로 필요
- 링크 형식(`v: 1`)은 사이트 `lib/importLink.ts`와 봇 `src/importLink.ts` 두 곳에 있다.
  레포가 분리돼 있으므로 한쪽만 바꾸면 조용히 깨진다 — 바꿀 때는 반드시 양쪽을 함께 고치고 `v`를 올릴 것
